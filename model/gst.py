...
class GST(nn.Module):
    def __init__(
        self,
    ):  # , ref_enc_filters, n_mel_channels, ref_enc_gru_size, token_num, token_embedding_size, num_heads, ref_enc_gru_size):
        super().__init__()
        ...
        self.stl = STL(
            token_num=10, token_embedding_size=256, num_heads=8, ref_enc_gru_size=128
        )

    def forward(self, mels, weights=None):
        out = self.reference_encoder(mels)
        out = self.stl(out)
        return out
...
if __name__ == "__main__":
    ...
    mels = torch.rand(64, 80, 100)

    out = gst(mels)
    print(out.shape)  # 64,1,256
